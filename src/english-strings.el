...
Email-address:\n\nOther:\t")
    ...
    (what-conf-to-delete . "Delete which conference/user: ")
    ...
    (review-text-no . "Review article number %#1d.\n")
    (no-comment-to . "There is no commented article.\n")

    (who-letter-to . "Send a letter to who? ")
    ...
    (where-to-add-self . "Which conference do you want to become a member of? ")
    ...
    (anyone-member . "May anybody become a member? ")
    ...
    (session-ended . "
********************************
 The Lyskom session is finished
********************************\n")
    ...
    (unsubscribe-failed . "\nDidn't work. Perhaps %#1s never was a member of %#2s?\n")
    ...
    (change-supervisor-nope . 
     "\nDidn't work. Perhaps you are not permitted to change the supervisor for %#1s?\n")
    ...
    (created-by . "Created by %#1s %#2s\n")
    (created-at . "Created:%35#1s\n")
    (members .    "Number of members: %25#1d\n")
    (garb-nice . "Expiration time (in days):%14#1d\n")
    (lowest-local-no . "Lowest local number: %8#1d\n")
    (highest-local-no . "Highest local number: %8#1d\n")
    (last-text-time . 
     "Time of last article: %18#1s (accordning to your cache)\n")
    (no-of-motd . "Note on the door in article: %12#1d\n")
    (superconf-is-no-name . "Superconference:       %25#1d %#2s\n")
    (permitted-submitters-no-name . "Admitted authors:%22#1d %#2s\n")
    (supervisor-is-no-name . "Organizer:     %25#1d %#2s\n")
    ...
    (created-persons . "Users created:%24#1d\n")
    (created-texts . "Articles created:%26#1d\n")
    (created-lines . "Lines created:%27#1d\n")
    (created-chars . "Characters created:%20#1d\n")
    (no-of-sessions . "Session count:%25#1d\n")
    (present-time-d-h-m-s . "Total presence:%19#1d d %02#2d:%02#3d:%02#4d\n")
...
cache says)\n")
    ...
    (who-to-send-message-to . "Send message to who? (Everyone) ")
  ...
  "Unable to send the message. Perhaps the recipient is not logged on.\n")
    ...
    (buggreport-description . "IThis is what I was doing:
...
Mark the envelope with \"LysKOM bug report\"\n\n")
    ...
    (no-get-pers . "You are not permitted to access that person.\n")
    ...
    ; From edit-text.el:
    (press-C-c-C-c . "Enter C-c C-c to submit the article.")
    (recipient . "Recipient:")
    (carbon-copy . "Carbon copy:")
    (text-mass . "%#4s%#1s\n%#2s\n%#3s")
    (comment-to-by . "%#1s to article %#2d%#3s.\n")
    (already-sent . "You have already submitted this article. Submit it anyway? ")
    (subject . "Subject: ")
...
; used by lyskom-return-time
    ...
    (Recipient . "Recipient")
    (Extra-recipient . "CC")
    (send-at . "    Submitted:     %#1s\n")
    (sent-by . "    Submitted by ")
...
===========================================================\n")
  ...
  Prio Conf# Conference\n")
    ...
    (save-on-file-q . "Which article to save in file: (%#1s) ")
...
Error message: %#1s**************************************************")
    ...
    (footnote . "Footnote")
    (by . " by %#1s")
    (text-created .  "Article number %#1d has been created.\n")
    )
...
(defconst lyskom-commands
  '(
    ...
    (kom-force-logout           "Throw out (session)")
    )
...
(defconst kom-tell-phrases
  '(
    ...
    (kom-tell-send		"Trying to submit a article.")
    (kom-tell-login		"Logging in.")
    (kom-tell-read		"Reading.")
    (kom-tell-1st-pres		"Writing the first presentation.")
    (kom-tell-write-comment	"Writing a comment.")
    (kom-tell-write-footnote	"Writing a footnote.")
    (kom-tell-write-letter	"Writing a letter.")
    (kom-tell-write-reply	"Writing a personal reply.")
    (kom-tell-write-text	"Writing a article.")
    (kom-tell-conf-pres		"Writing the presentation for a new conference.")
    (kom-tell-recover		"Performing a reset. Sigh.")
    (kom-tell-wait		"Waiting.")
    (kom-tell-regret		"Decides to throw away the article.")
    (kom-tell-review		"Reviewing.")
    (kom-tell-change-name       "Takes on a new name.")
    (kom-tell-change-supervisor "Changes the supervisor for something.")
    )
